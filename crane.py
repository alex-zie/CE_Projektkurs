...
class crane_1(Truss):
    ...
    def __init__(self, height, length, ls, A, rho, E, p=True, max_bar_length=-1):
        ...
        if p:
            print("Creating crane with cuboid tower with " + str(self.nST) + " segments of length " + str(
                self.ls) + " and pyramidal jib with " + str(self.nSA) + " segments.")
...
class crane_2_1(Truss):
    ...
    def __init__(self, height, length, ls, A, rho, E, p=True):
        ...
        if p:
            print("Creating crane with zig-zag tower with " + str(self.nST) + " segments of length " + str(
                self.ls) + " and pyramidal jib with " + str(self.nSA) + " segments.")
        self.make_tower(nodes, bars)
...
def cur_offset(nodes):
    """
    Returns the current length of the nodes list, used for the crane structure
    """
    return len(nodes)
