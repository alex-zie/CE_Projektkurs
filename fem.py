...
def TrussAnalysis(truss: truss.truss, A, rho,E) -> (np.ndarray, np.ndarray, np.ndarray) :
    nodes = truss.nodes
    bars = truss.bars
    NN = len(nodes)
    NE = len(bars)
    ...
    freeDOF = truss.DOFCON.flatten().nonzero()[0]  # Prüfe, welche Knoten FG > 0 haben
    supportDOF = (truss.DOFCON.flatten() == 0).nonzero()[0]  # Knoten mit Lagern
